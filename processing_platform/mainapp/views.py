...
def easy_growth_view(request):
    """
    The function calls the api (not created yet) and with the reponse it splits the data into lists for each variabel.
    ...
    try:
        ...
        return render(
            request,
            "mainapp/easy_growth.html",
            {
                "error": f"error with api: {e}",
                "graphs": [],
            },
        )
    ...
    exclude_keys = {"TS", "crop", "PID", "soilType", "state"}
    labels = [entry["TS"] for entry in data if "TS" in entry]

    all_keys = set().union(*(entry.keys() for entry in data)) - exclude_keys
    ...
    for key in sorted(all_keys):
        ...
        if values:
            graphs.append({"label": key, "data": values})
...
def review_drone_flights(request):
    ...
    if os.path.exists(csv_file_path):
        try:
            ...
            df_renamed["date_display"] = pd.to_datetime(df["Flight Date"]).dt.strftime(
                "%B %d, %Y"
            )
            ...
            if selected_flight_type != "all":
                df_renamed = df_renamed[
                    df_renamed["image_type"] == selected_flight_type
                ]
            if selected_drone_pilot != "all":
                df_renamed = df_renamed[
                    df_renamed["drone_pilot"] == selected_drone_pilot
                ]
            if date_from:
            ...
            unique_flight_types = (
                df["Route type (MS, 3D, Thermal, RGB)"].dropna().unique()
            )
            unique_drone_pilots = df["Drone Pilot"].dropna().unique()

            fields += [{"value": f, "label": f} for f in sorted(unique_fields)]
            flight_types += [
                {"value": f, "label": f} for f in sorted(unique_flight_types)
            ]
            drone_pilots += [
                {"value": p, "label": p} for p in sorted(unique_drone_pilots)
            ]
